...
)
{
    ...
    for( i = 0; i < 16; i++ )
    {
        sprintf( &md5result[ i * 2 ], "%02x", resblock[ i ] & 0x00ff );
    }
}
...
md5_finish_ctx (struct md5_ctx *ctx, void *resbuf)
{
  ...
  memcpy (&ctx->buffer[bytes], ctx->fillbuf, pad);
...
/**
 * Process any bytes remaining in the buffer once the block processing is
 * finished. If the MD5 digest is computed in-memory, the entire buffer may
 * be considered the "remaining bytes."
 ...
 */
int
MD5DigestStream(FILE *stream, char *md5sum )
{
